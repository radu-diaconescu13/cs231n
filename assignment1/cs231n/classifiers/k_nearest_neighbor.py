...
class KNearestNeighbor(object):
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            dists[i, :] = np.sqrt(summed_squared_diffs)
            pass
    ...
    def compute_distances_no_loops(self, X):
        ...
        # print(f'X_train has shape {self.X_train.shape}')
        # print(f'X_test has shape {X.shape}')
        product = np.multiply(-2, np.matmul(X, np.transpose(self.X_train)))
        # print(product.shape)

        X_square = np.reshape(X_square, [num_test, 1])
        # print(X_square.shape)

        dists = X_square + X_train_square + product
        dists = np.sqrt(dists)
        pass
